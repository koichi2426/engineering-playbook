...
> しかし、例えば `file_data.go` のように、フレームワーク固有の型（echo.Context や multipart.FileHeader など）から独立させるため抽象的な定義（interface）としてしか表現できないケースがあります。
>
...
* `src/infrastructure/` (インフラ層)
	- 責務: 外部技術とdomainインターフェースの具体実装
	- 内容:
		- `database/`: 各DB技術のリポジトリ実装（各フォルダ直下に必ず config.go）
