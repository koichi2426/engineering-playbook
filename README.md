...
最終的なゴールは、このPlaybookを人間が参照するだけでなく、**GitHub Copilotに「独自のルールブック」として読み込ませ**、開発作業を自動化することです。
...
設計や実装で迷った際の「辞書」として、また新しいメンバーの「教科書」として参照してください。
（例: 「APIのエラーレスポンスはどうだっけ？」→ `2_Concepts/error_handling.md` を確認）
